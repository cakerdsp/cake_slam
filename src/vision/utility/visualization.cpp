...
std::shared_ptr<tf2_ros::TransformBroadcaster> pub_tf_broadcaster;
...
void registerPub(rclcpp::Node::SharedPtr n)
{
    ...
    pub_tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(n);
...
{
    if (!pub_tf_broadcaster || estimator.solver_flag != Estimator::NON_LINEAR)
        return;

    geometry_msgs::msg::TransformStamped transform, transform_cam;

    // body frame
    const Eigen::Vector3d correct_t = estimator.states_[WINDOW_SIZE].pos_end;
    const Eigen::Quaterniond correct_q(estimator.states_[WINDOW_SIZE].rot_end);
    ...
    pub_tf_broadcaster->sendTransform(transform);
    ...
    const Eigen::Quaterniond q_ic(estimator.ric[0]);
    transform_cam.transform.rotation.x = q_ic.x();
    transform_cam.transform.rotation.y = q_ic.y();
    transform_cam.transform.rotation.z = q_ic.z();
    transform_cam.transform.rotation.w = q_ic.w();
    pub_tf_broadcaster->sendTransform(transform_cam);
