...
namespace
{
const sensor_msgs::msg::PointField *findField(const sensor_msgs::msg::PointCloud2 &msg, const std::vector<std::string> &names)
{
...
bool readFieldAsDouble(const uint8_t *point_data, const sensor_msgs::msg::PointField &field, double &value)
{
...
double normalizeLivoxPointTimeMs(double raw_time, double min_time, double max_time, size_t index, size_t count, int scan_rate)
{
  ...
  if (span > 1.0e6) return rel / 1.0e6; // nanoseconds to milliseconds
  if (span > 1.0e3) return rel / 1.0e3; // microseconds to milliseconds
  if (span <= 1.0) return rel * 1.0e3;  // seconds to milliseconds
  return rel;                           // already milliseconds
}
} // namespace
